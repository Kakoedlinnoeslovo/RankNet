...
class Network:
	...
	def ranknet(y_true, y_pred):
		loss_pair = K.mean(K.log(1. + K.exp(-(y_true * y_pred - (1 - y_true) * y_pred))), axis=-1)
		loss_mse = K.mean(K.log((y_true - y_pred)**2), axis = -1) * 0.001

		return loss_mse  + loss_pair
...
def main():
	X_train, y_train, Query = readDataset("data/train.data.cvs", howlines=10000, isall=True)
	...
	model_.model.compile(optimizer=optimizer, loss=model_.ranknet)
